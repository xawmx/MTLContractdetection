...
class MTDataset_Split:
    def __init__(self, contract_data, func_data, contract_label, func_label, num_class):
...
def read_data_from_file(contract_filename, func_filename, contract_label_file, func_label_file):
        ...
        for iter in range(max_iter_epoch * max_epoch):
            ...
            train_step.run(feed_dict={d1: d2 for d1, d2 in
                                      zip([learning_rate, gradient_clipping_option, gradient_clipping_threshold, inputs,
                                          ...
                                          [0.002/ (num_iter + 1), 0, -5., sampled_data, sampled_label, sampled_task_ind,
...
def main_process(contract_filename, func_filename, graph_label_file, func_label_file, train_size, hidden_dim, batch_size, reg_para, max_epoch, use_gpu, gpu_id='0', activate_op=1):
    if use_gpu == 1:
        os.environ['CUDA_VISIBLE_DEVICES'] = gpu_id
    else:
        os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'
    contract_data, func_data, contract_label, func_label, num_task, num_class = read_data_from_file(contract_filename, func_filename, graph_label_file, func_label_file)
    # data_split = MTDataset_Split(data, label, task_interval, num_class)
    data_split = MTDataset_Split(contract_data, func_data, contract_label, func_label, num_class)
    ...
    y_loss_f = []
    y_acc_f = []
    for i in range(len(y_train_loss)):
...
mean_errors = main_process(graph_feature_file, func_feature_file, graph_label_file, func_label_file, train_size, hidden_dim, batch_size, reg_para, max_epoch, use_gpu, gpu_id, activate_op)
