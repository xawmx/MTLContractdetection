...
class MTDataset:
    ...
    def get_next_batch(self):
        sampled_data = np.zeros([256, self.data_dim], dtype=np.float32)
        sampled_label = np.zeros([256, self.num_class], dtype=np.int32)
        sampled_task_ind = np.zeros([1, 256], dtype=np.int32)
        sampled_label_ind = np.zeros([1, 256], dtype=np.int32)
        temp_index = []
        for i in range(16):
            for j in range(16):
                temp_index.append(random.randint(i*64, (i+1)*64-1))
        sampled_data = self.data[temp_index]
        sampled_label = list(self.label[0][temp_index])
        temp_label = []
        for i in range(len(sampled_label)):
            if sampled_label[i] == 0:
                temp_label.append([1, 0])
            else:
                temp_label.append([0, 1])
        sampled_label = np.array(temp_label)
        sampled_task_ind_temp = []
        for i in range(len(temp_index)):
            sampled_task_ind_temp.append(0)
        temp_content = []
        temp_content.append(sampled_task_ind_temp)
        temp_content = np.array(temp_content)
        sampled_task_ind = temp_content
        sampled_label_ind = np.array(temp_index)
        sampled_task_ind = np.reshape(sampled_task_ind, (1, 256))
...
class MTDataset_Split:
    ...
    def split(self, train_size, contract_edge):
        if train_size < 1:
        ...
        index = [5, 6, 7, 9, 10, 11, 13, 15, 16, 17, 21, 23, 24, 25, 31, 32]

        for i in range(len(index)):
            np.random.shuffle(contract_edge[index[i]])
            graph_index_train = np.array(contract_edge[index[i]][0:64])
            graph_index_test = np.array(contract_edge[index[i]][-33:-1])
            traindata = np.concatenate((traindata, self.data[graph_index_train, :]), axis=0)
            trainlabel = np.concatenate((trainlabel, self.label[:, graph_index_train]), axis=1)
            testdata = np.concatenate((testdata, self.data[graph_index_test, :]), axis=0)
            testlabel = np.concatenate((testlabel, self.label[:, graph_index_test]), axis=1)

        train_task_interval[0, 1] = 0
        test_task_interval[0, 1] = 0
        train_task_interval[0, 1] = trainlabel.size
        test_task_interval[0, 1] = testlabel.size
        # for i in range(self.num_task):
        #     for j in range(self.num_class):
...
def read_data_from_file(contract_filename, func_filename, contract_label_file, func_label_file, graph_edge_file):
    ...
    for n in range(len(contract_subset_nodup)):
        while len(contract_subset_nodup[n]) < 64:
            contract_subset_nodup[n] = contract_subset_nodup[n] + contract_subset_nodup[n]
    ...
    return contract_data, contract_label, num_task, num_class, contract_subset_nodup
...
def compute_train_loss(i, feature_representation, hidden_output_weight, inputs_data_label, inputs_task_ind, inputs_num_ins_per_task, train_loss):
    train_loss += tf.div(tf.losses.softmax_cross_entropy(tf.expand_dims(inputs_data_label[i, :], 0),
                                                         tf.matmul(tf.expand_dims(feature_representation[0][i % 256][:], 0),
...
def compute_adjacency_matrix(hidden_features, inputs_data_label, num_task):
    ...
    # # 拼接起来
    temp_matrixs = np.zeros((256, 256))
    for i in range(16):
        for j in range(16):
            for m in range(16):
                temp_matrixs[i*16 + j][i*16 + m] = 1 / 16
    adjacency_matrixs = tf.convert_to_tensor(temp_matrixs, dtype=tf.float32)
...
def get_feature_representation(inputs, input_hidden_weights, hidden_features, adjacency_matrix, num_task, num_class, activate_op,
    ...
    new_inputs_data_label = change_datastruct(inputs_data_label, num_task)
    # 将标签也按任务数分好
    new_adjacency_matrix = np.zeros((256, 256))
    for i in range(16):
        for j in range(16):
            for m in range(16):
                new_adjacency_matrix[i*16 + j][i*16 + m] = 1 / 16
    new_adjacency_matrix = tf.convert_to_tensor(new_adjacency_matrix,dtype=tf.float32)
    ...
    new_hidden_representation = tf.reshape(new_hidden_representation, [16, 16, -1])
    ...
    for i in range(16):
        ...
        feature_representation_1 = tf.concat([
            hidden_features[i * 16: (i + 1) * 16], tf.stack([new_task_embedding_vectors[i] for _ in range(16)])], 1)
        feature_representations.append(feature_representation_1)
    # 得到链接后特征，主要是隐式表征链接task表征链接类表征
    feature_representations = tf.stack(feature_representations)
    # 实际上返回的是最后增强的特征
    return tf.reshape(feature_representations, [num_task, -1, hidden_features.shape[-1] + F_pie_t])
...
def get_embedding_vec(traindata, input_hidden_weights, first_task_att_w, first_class_att_w, task_attention_weight,
    ...
    labels = []
    for i in range(traindata.shape[0]):
        inputs.append(list(traindata[i]))
        features.append(list(train_hidden_features[i]))
        labels.append(list(train_label_matrix[i]))
    inputs = np.array(inputs)
    features = np.array(features)
    labels = np.array(labels)
    task_embedding_vectors = []

    # dist_matrix = -compute_pairwise_dist_np(np.stack(features))
    ...
    adjacency_matrix = np.zeros((1024, 1024))
    for i in range(16):
        for j in range(64):
            for m in range(64):
                adjacency_matrix[i * 64 + j][i * 64 + m] = 1/64

    new_features = np.tanh(np.add(np.matmul(inputs, input_hidden_weights),
                                  np.matmul(adjacency_matrix, features)))
    ...
    new_features = np.reshape(new_features, (16, 64, -1))
    task_embedding_vectors = np.max(new_features, 1)
    # task_embedding_vector = np.reshape(task_embedding_vector, (16, 1, 300))
    ...
    task_attention_values = np_softmax(get_normed_distance_np(task_embedding_vectors))
    ...
    task_attention_values = np_softmax(get_normed_distance_np(new_task_embedding_vectors))
...
def get_new_hidden_features(test_hidden_rep, task_embedding_vectors, hidden_output_weight, test_task_ind, num_task, num_class):
    temp_test_hidden_rep = []
    for i in range(test_hidden_rep.shape[0]):
        temp = np.concatenate([test_hidden_rep[i], task_embedding_vectors[int(i / 32)]], 0)
        temp_test_hidden_rep.append(temp)
    temp_test_hidden_rep = np.stack(temp_test_hidden_rep)
    test_hidden_rep = temp_test_hidden_rep
...
def MTL_GRAPH(traindata, trainlabel, train_task_interval, dim, num_class, num_task, hidden_dim, batch_size, reg_para,
    ...
    hidden_output_weight = tf.Variable(tf.truncated_normal(
        [num_task, hidden_dim + F_pie_t, num_class], dtype=tf.float32, stddev=1e-1))
    # 多任务层，分别分类
        ...
        for iter in range(max_iter_epoch * max_epoch):
            ...
            train_step.run(feed_dict={d1: d2 for d1, d2 in
                                      zip([learning_rate, gradient_clipping_option, gradient_clipping_threshold, inputs,
                                          ...
                                          [0.0002, 0, -5., sampled_data, sampled_label, sampled_task_ind, np.ones([1, 1])*256])})
            # 0.02 / (1 + num_iter)
            ...
            if iter % 4 == 0 and num_iter % 1 == 0:
                ...
                task_embedding_vectors = get_embedding_vec(traindata, input_hidden_weights.eval(), first_task_att_w.eval(), first_class_att_w.eval(), task_attention_weight.eval(), class_attention_weight.eval(),
                ...
                new_test_hidden_rep = get_new_hidden_features(test_hidden_rep, task_embedding_vectors, hidden_output_weight.eval(), test_task_ind, num_task, num_class)
...
def main_process(contract_filename, func_filename, graph_label_file, func_label_file, graph_edge_file, train_size, hidden_dim, batch_size, reg_para, max_epoch, use_gpu, gpu_id='0', activate_op=1):
    ...
    contract_data, contract_label, num_task, num_class, contract_edge = read_data_from_file(contract_filename, func_filename, graph_label_file, func_label_file, graph_edge_file)
    ...
    traindata, trainlabel, train_task_interval, testdata, testlabel, test_task_interval = data_split.split(train_size, contract_edge)
